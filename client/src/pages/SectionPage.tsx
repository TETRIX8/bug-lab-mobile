...
export default function SectionPage({ section }: { section: Section }) {
  ...
  return <main className="bug-lab-shell"><div className="grain" /><Header current="bugs" /><section className="subpage container bugs-page"><div className="section-heading"><div><div className="eyebrow">THE COLLECTION / 20 TESTS</div><h1>Каталог<br /><em>приколов.</em></h1></div><p>Каждая карточка — отдельная проверка. Нажимай всё, что выглядит подозрительно.</p></div><div className="bug-grid">{BUGS.map((bug) => { const done = passed.includes(bug.id); return <article key={bug.id} className={`bug-card bug-${bug.color}`}><div className="bug-card-top"><span className="bug-number">#{String(bug.id).padStart(2, "0")}</span><span className="bug-tag">{bug.tag}</span><span className="bug-icon"><bug.icon /></span></div><h3>{bug.title}</h3><p>{bug.detail}</p><div className="bug-card-bottom"><span className="bug-joke">{done ? "Поймано. Но почему работает?" : bug.joke}</span><button className={`small-action ${done ? "is-done" : ""}`} onClick={() => { setPassed((current) => current.includes(bug.id) ? current : [...current, bug.id]); setToast(`${bug.title}: поймано`); }} aria-label={`Проверить ${bug.title}`}>{done ? <Check size={17} /> : <ChevronRight size={18} />}</button></div></article>; })}</div><div className="catalog-footer"><span>{passed.length} / 20 поймано</span><span>{toast}</span><a href="/lab">Перейти в лабораторию <ChevronRight size={14} /></a></div></section><Footer /></main>;
}
